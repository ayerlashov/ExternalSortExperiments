...
namespace BigFileSorter
{
    public class Program
    {
        ...
        {
            var sourceFilePathArg = new Argument<FileInfo>("sourceFilePath", "The unsorted file.")
            {
                Arity = ArgumentArity.ExactlyOne
            };
            var tempDirectoryArg = new Argument<DirectoryInfo>("tempFileDirectory", "Where to write temp files.")
            {
                Arity = ArgumentArity.ExactlyOne
            };

            var targetFilePathArg = new Argument<FileInfo>("targetFilePath", "Where to write the sorted file.")
            {
                Arity = ArgumentArity.ExactlyOne
            };

            sourceFilePathArg.ExistingOnly();
            Option<int> blockSizeThresholdOpt = MakeBlockSizeThresholdOption();
            Option<int> maxMergeFactorOpt = MakeMaxMergeFactorOption();
            Option<int> maxMemoryChunksOpt = MakeMemoryChunkOption();
        ...
        private static Option<int> MakeMaxMergeFactorOption()
        {
            var option = new Option<int>(
                "-k",
                () => 100,
                "The k in k-way merge. n > 2. The merges are buffered. Be reasonable.")
            {
                Arity = ArgumentArity.ZeroOrOne
            };

            option.AddValidator(option =>
            {
                var val = option.GetValueOrDefault<int>();

                if (val is < 1 or > 1000)
                {
                    option.ErrorMessage = "Value must be greater than 1.";
                }
            });
            return option;
        }

        private static Option<int> MakeMemoryChunkOption()
        {
            var maxMemoryChunksOpt = new Option<int>(
                "-c",
                () => 4,
                "The maximum number of chunks permitted to be simultaneously loaded into memory. n > 1. Be reasonable.")
            {
                Arity = ArgumentArity.ZeroOrOne
            };

            maxMemoryChunksOpt.AddValidator(option =>
            {
                var val = option.GetValueOrDefault<int>();
            ...
            return maxMemoryChunksOpt;
        }

        private static Option<int> MakeBlockSizeThresholdOption()
        {
            var blockSizeThresholdOpt = new Option<int>(
                "-b",
                () => DefaultMegabyteThreshold,
                "How big first pass blocks can be in MB. Must be in range [1, 1000].")
            {
                Arity = ArgumentArity.ZeroOrOne
            };
            ...
            int maxMemoryChunks)
        {
            ...
            sw.Start();
            new ExternalMergeSorter(tempDirectory.FullName, blockSizeThreshold << 20, maxMergeFactor, maxMemoryChunks)
